defmodule Pizza.Adapters.PizzaProjectionTest do
  ...
  describe "saving pizza" do
    ...
    test "does not overwrite a pizza, even if data have changed, given event version has already been received", %{cloud_event: cloud_event, pizza_projection: pizza_projection, pizza: pizza} do
      changed = %CorePizza{
        id: pizza.id,
    ...
    test "updates a pizza", %{cloud_event: cloud_event, pizza_projection: pizza_projection, pizza: pizza} do
      changed = %CorePizza{
        id: pizza.id,
        name: pizza.name,
        price: 10
  ...
  describe "retriving a list of all pizzas" do
    setup %{pizza_projection: pizza_projection, cloud_event: cloud_event, time: time} do
      ...
      funghi_event = CloudEvent.new_v1(
        :test,
        :create_pizza,
      ...
      pepperoni_event = CloudEvent.new_v1(
        :test,
        :create_pizza,
      ...
      Enum.each([cloud_event, funghi_event, pepperoni_event], fn event -> PizzaProjection.save(pizza_projection, event) end)
      {:ok, funghi: funghi, pepperoni: pepperoni}
    end

    test "retrieves all pizzas, sorted ascending alphabetically", %{pizza_projection: pizza_projection, pizza: pizza, funghi: funghi, pepperoni: pepperoni} do
    ...
    test "retrieves all pizzas, sorted descending alphabetically", %{pizza_projection: pizza_projection, pizza: pizza, funghi: funghi, pepperoni: pepperoni} do
    ...
    test "retrieves all pizzas, sorted ascending by updated time", %{pizza_projection: pizza_projection, pizza: pizza, funghi: funghi, pepperoni: pepperoni} do
      {:ok, time_ascending} = PizzaProjection.list_chronological(pizza_projection, :asc)
      assert time_ascending == [pizza, funghi, pepperoni]
    end

    test "retrieves all pizzas, sorted descending by updated time", %{pizza_projection: pizza_projection, pizza: pizza, funghi: funghi, pepperoni: pepperoni} do
      {:ok, time_descending} = PizzaProjection.list_chronological(pizza_projection, :desc)
      assert time_descending == [pepperoni, funghi, pizza]
    end

    test "retrieves all pizzas, sorted ascending by price", %{pizza_projection: pizza_projection, pizza: pizza, funghi: funghi, pepperoni: pepperoni} do
      {:ok, price_ascending} = PizzaProjection.list_by_price(pizza_projection, :asc)
      assert price_ascending == [pizza, funghi, pepperoni]
    end

    test "retrieves all pizzas, sorted descending by price", %{pizza_projection: pizza_projection, pizza: pizza, funghi: funghi, pepperoni: pepperoni} do
